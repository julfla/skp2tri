...
/*! \mainpage SketchUp API

# Overview

Welcome to the C API for SketchUp. SLAPI is an interface for reading and writing
data to and from SketchUp models. It can be used within the SketchUp plugin
architecture to create an importer or an exporter plugin and then be available
directly within the SketchUp application as an import or export option. It can
also be used in any standalone application to read and write SketchUp model data
from and to .SKP files, so you can create an importer or exporter for .SKP files
within your own application.

This documentation contains reference material for all functions available in
the API, as well as descriptions of data structures and enumerations used by the
API functions.

Included in the SDK package are two C++ sample projects: skp_to_xml and
xml_to_skp. These projects demonstrate how the SketchUp C API can be used within
the SketchUp plugin architecture to create importers and exporters meant to be
hosted directly by SketchUp. The XML schema used by the samples is only for
...
# Memory Management in the SketchUp API
The SketchUp API does not automatically track and dispose of objects that are
